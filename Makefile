...
VERSION = $(shell python3 -c 'from cars_resample import __version__; print(__version__)')
...
install: venv git  ## install the package in dev mode in virtualenv
	@test -f resample*.so || echo "Install cars-resample package from local directory"
	@test -f resample*.so || ${VENV}/bin/python -m pip install -e .[dev]
	@test -f .git/hooks/pre-commit || echo "Install pre-commit"
	@test -f .git/hooks/pre-commit || ${VENV}/bin/pre-commit install -t pre-commit
	@chmod +x ${VENV}/bin/register-python-argcomplete
	@echo "cars-resample ${VERSION} installed in dev mode in virtualenv ${VENV} with documentation"
	@echo " cars-resample venv usage : source ${VENV}/bin/activate"
...
coverage: install ## check code coverage quickly with the default Python
	@${VENV}/bin/coverage run --source cars_resample -m pytest
...
format/isort: install  ## run isort formatting (depends install)
	@echo "+ $@"
	@${VENV}/bin/isort cars_resample tests

.PHONY: format/black
format/black: install  ## run black formatting (depends install)
	@echo "+ $@"
	@${VENV}/bin/black cars_resample tests
...
lint/isort: ## check imports style with isort
	@echo "+ $@"
	@${VENV}/bin/isort --check cars_resample tests

.PHONY: lint/black
lint/black: ## check global style with black
	@echo "+ $@"
	@${VENV}/bin/black --check cars_resample tests

.PHONY: lint/flake8
lint/flake8: ## check linting with flake8
	@echo "+ $@"
	@${VENV}/bin/flake8 cars_resample tests

.PHONY: lint/pylint
lint/pylint: ## check linting with pylint
	@echo "+ $@"
	@set -o pipefail; ${VENV}/bin/pylint cars_resample tests --rcfile=.pylintrc --output-format=parseable | tee pylint-report.txt # pipefail to propagate pylint exit code in bash

.PHONY: lint/mypy
lint/mypy: ## check linting type hints with mypy
	@echo "+ $@"
	@${VENV}/bin/mypy cars_resample tests
